...
void DrawCharSelect(MenuSystemState *currentState, CharClass *selected) {
    ...
    DrawText(title, (int)(sw / 2 - tw / 2), 50, tf,
    ...
    for (int i = 0; i < CHAR_COUNT; i++) {
        ...
        Color bg = sel ? (Color){ 25, 45, 25, 230 }
    ...
    if (DrawButton((Rectangle){ btnX, btnY, btnW, btnH },
                   "START GAME", (Color){ 120, 220, 120, 255 }))
        *currentState = STATE_GAMEPLAY;

    if (DrawButton((Rectangle){ btnX, btnY + 54, btnW, btnH },
                   "BACK", (Color){ 200, 180, 100, 255 }))
...
void DrawGameOver(MenuSystemState *currentState, GameState *gs) {
    ...
    int tf = 36;
    int tw = MeasureText(title, tf);
    DrawText(title, (int)(sw / 2 - tw / 2), py + 20, tf,
             (Color){ 220, 40, 40, 255 });

    const char *sub = "You died in the dark...";
    int sw2 = MeasureText(sub, 13);
    DrawText(sub, (int)(sw / 2 - sw2 / 2), py + 70, 13,
             (Color){ 180, 140, 140, 255 });
    ...
    if (DrawButton((Rectangle){ btnX, py + 160, btnW, btnH },
